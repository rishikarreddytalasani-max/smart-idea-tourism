...
plannerForm.addEventListener("submit", (e) => {
  e.preventDefault();
  const destRaw = document.getElementById("pDest").value.trim();
  ...
  const startTimeStr = (document.getElementById("pStartTime")?.value || "09:00").trim();
  ...
  if (!destKey) {
    itinerary.innerHTML = `<h3>⚠️ Destination not found</h3><p>Please type one of: <b>${Object.keys(placeData).join(", ")}</b></p>`;
  ...
  let html = `<h3>✨ ${days}-day itinerary for ${destKey}</h3><ul>`;
  let idx = 0;
  for (let d = 1; d <= days; d++) {
    html += `<li><strong>Day ${d}</strong> — ${start} → ${destKey}</li>`;

    // Start each day from the selected start time
    let cur = { h: isNaN(sh) ? 9 : sh, m: isNaN(sm) ? 0 : sm };

    for (let s = 0; s < perDay && idx < spots.length; s++, idx++) {
      const visitStart = cur;
      const visitEnd = addMinutes(cur.h, cur.m, 90); // 1h 30m per spot (demo)
      html += `<li>&nbsp;&nbsp;📍 ${spots[idx]} (${fmtTime(visitStart.h, visitStart.m)} – ${fmtTime(visitEnd.h, visitEnd.m)})</li>`;
      cur = addMinutes(visitEnd.h, visitEnd.m, 30); // 30m travel buffer (demo)
    }

    html += `<li>&nbsp;&nbsp;🍽️ Lunch break</li>`;
  }
...
// ---------- Directions + Traffic (demo via Google Maps) ----------
...
const savedCoupon = localStorage.getItem("smartTourCoupon");
if (couponBox && savedCoupon) couponBox.textContent = savedCoupon;
...
if (submitReviewBtn) {
  submitReviewBtn.addEventListener("click", () => {
    ...
    localStorage.setItem("smartTourCoupon", coupon);
    if (couponBox) couponBox.textContent = coupon;
    alert(`Thanks, ${name}! Rating: ${rating}/5. Your coupon: ${coupon}`);
...
if (clearCouponBtn) {
  clearCouponBtn.addEventListener("click", () => {
    localStorage.removeItem("smartTourCoupon");
    if (couponBox) couponBox.textContent = "No coupon yet";
  });
}